...
inline void getAllSubsetsRec(const std::vector<T>& set, std::vector<std::vector<T>>& res,
    const std::vector<T>& accum, unsigned index, long unsigned count)
{
...
template<>
inline void getAllSubsetsRec(const Rots& set, std::vector<Rots>& res, const Rots& accum, unsigned index, long unsigned count) {
    if ((accum.size() == count) || (index >= set.size())) {
        res.push_back(accum);
        return;
    }
...
template<typename T>
void getAllSubsets(const std::vector<T>& from, std::vector<std::vector<T>>& res, long unsigned count) {
