...
class condition_variable_any
{
    ...
    struct unlock_guard
    {
        unlock_guard( Lockable & mtx_ ) : mtx( mtx_ )
        {
            mtx.unlock();
        }
        ~unlock_guard()
...
public:
    ...
    condition_variable_any() : internals{ std::make_shared< cv_internals >() } {}
                     ...
                     Predicate pred )
    {
        ...
        while ( !pred() ) {
            bool shouldStop;
            {
                ...
                shouldStop = ( local_internals->cv.wait_until( second_internal_lock, abs_time )
...
private:
    ...
    // If this happens with condition_variable_any, that waiting thread will attempt to lock the destructed mutex mut.
...
//*****************************************************************************
//* implementation of class condition_variable_any
//*****************************************************************************
...
// - false otherwise (i.e. on interrupt)
template < class Lockable, class Predicate >
inline bool condition_variable_any::wait( Lockable & lock, stop_token stoken, Predicate pred )
{
...
// - false otherwise (i.e. on timeout or interrupt)
template < class Lockable, class Clock, class Duration, class Predicate >
inline bool condition_variable_any::wait_until(
        Lockable & lock,
        stop_token stoken,
        const std::chrono::time_point< Clock, Duration > & abs_time,
        Predicate pred )
...
// - false otherwise (i.e. on timeout or interrupt)
template < class Lockable, class Rep, class Period, class Predicate >
inline bool condition_variable_any::wait_for( Lockable & lock,
                                              stop_token stoken,
                                              const std::chrono::duration< Rep, Period > & rel_time,
                                              Predicate pred )
{
