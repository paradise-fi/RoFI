...
def all_combinations(matrix: Dict[str, List[Any]]) -> Iterable[Dict[str, Any]]:
    pair_matrix = [[(name, value) for value in matrix[name]]
                   for name in matrix]
    return [dict(pairs) for pairs in itertools.product(*pair_matrix)]


@click.command(help='Generate task set from given format and args from TASK_ARGS_FILE (in json format) and cmd and matrix args from CMD_ARGS_FILE')
...
def gen_tasks_matrix(task_args_file: TextIO, cmd_args_file: TextIO, raw_args: bool):
    ...
    if not raw_args:
        task_args = [{name: safe_as_arg(args[name]) for name in args}
                     for args in task_args]
        matrix_args = {name: [safe_as_arg(value) for value in matrix_args[name]]
                       for name in matrix_args}

    tasks = [cmd.format_map(margs | targs)
             for targs in task_args for margs in all_combinations(matrix_args)]
